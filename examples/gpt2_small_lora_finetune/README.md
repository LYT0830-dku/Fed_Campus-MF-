This example provides a minimal runnable workflow for GPT-2 Small LoRA
...
- Pretrained weights are resolved from `MFT_MODEL_ROOT`,
  `GPT2_SMALL_MODEL_DIR`, or the local `pretrained/` fallback.
...
1. Prepare task JSONL training data with answer-label masks:
   ...
   Output: `runs/mmlu_jsonl_gpt2_s128/` under the repository root.
