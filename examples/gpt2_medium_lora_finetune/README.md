This example provides a minimal runnable workflow for GPT-2 Medium LoRA
...
- Pretrained weights are resolved from `MFT_MODEL_ROOT`,
  `GPT2_MEDIUM_MODEL_DIR`, or the local `pretrained/` fallback.
...
1. Prepare JSONL training data with masked labels:
   ...
   Output: `runs/mmlu_jsonl_gpt2_medium_s128/` under the repository root.
...
2. Run LoRA fine-tuning:
   ...
   Output: `outputs/lora_final.safetensors`.
...
- `run_train.sh`: `TRAIN_MODE`, `MMLU_JSONL`, `WT2_DATA_DIR`, `STEPS`,
  ...
  `GRAD_ACCUM`.
- `run_eval.sh`: `SPLIT`, `FEWSHOT`, `LORA_PATH`.
